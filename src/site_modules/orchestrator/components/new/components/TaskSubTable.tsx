import { Backdrop, Button, CircularProgress, createStyles, Grid, Hidden, makeStyles, TextField, Theme } from '@material-ui/core';
...
export function TaskSubTable(props: ITaskSubTableProps) {
  ...
  const formElements = Object.keys(selectedArguments).map((key) => {
    return <TextFieldComponent key={key} label={key} value={selectedArguments[key]} onChange={(event) => {handleChange(event.target.value, key);}}/>;
  });
